...
const updateSubTask = asyncHandler(async (req, res) => {
    ...
    const { title, isComplete } = req.body;
    const targetSubTaskId = subTaskId || subtaskId;

    const subTask = await Subtask.findById(targetSubTaskId);
    if (!subTask) {
    ...
    if (!updatedSubTask) {
        throw new ApiError(500, "Something went wrong while updating subtask");
    }
...
const deleteSubTask = asyncHandler(async (req, res) => {
    ...
    const deletedSubTask = await Subtask.findByIdAndDelete(targetSubTaskId);
    if (!deletedSubTask) {
        throw new ApiError(500, "Something went wrong while deleting subtask");
    }
